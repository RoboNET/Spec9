...
engine. Keep the engine and product model separate: the engine owns grammar,
...
normative documentation. Do not copy Spec9's `tools/`, `grammar/`,
`constitution.md`, or skills into the product repository.

Use the installed `spec9` executable when available. In local engine
development, a product may depend on the engine with a package-manager file
dependency; do not hardcode a machine-specific path into a reusable setup.
...
Read `<spec9-repo>/grammar/SYNTAX.md` for supported frontmatter and
`<spec9-repo>/constitution.md` for source-of-truth rules when profile choices
are unclear.
...
```bash
spec9 --spec-root <spec-root> --product-root <product-root> \
...
```bash
spec9 --spec-root <spec-root> --product-root <product-root> lint
spec9 --spec-root <spec-root> --product-root <product-root> graph
spec9 --spec-root <spec-root> --product-root <product-root> doctor --strict
spec9 --spec-root <spec-root> --product-root <product-root> quality --all
```
