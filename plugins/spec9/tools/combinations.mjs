...
// (docs/history/engine-audit-2026-08-30.md H3).
const DIM_RE = /^-\s*`([^`]+)`\s*[—–-]\s*(.+)$/;
...
function parseOneCombinationsSection(bodyLines, bodyStartLine, headings, idx) {
  ...
  for (let ln = start; ln < end; ln++) {
    ...
    // а с ним и всей проверки полноты таблицы (docs/history/engine-audit-2026-08-30.md H3). Заголовок
  ...
  for (let i = 2; i < tableLines.length; i++) {
    ...
    // иначе сверка идёт по смещённым значениям молча (docs/history/engine-audit-2026-08-30.md H3).
 ...
 * сообщалось ни строкой (docs/history/engine-audit-2026-08-30.md M6). Возвращает пустой массив, если ни
 ...
 * "**НЕ ОПРЕДЕЛЕНО**" (docs/history/engine-audit-2026-08-30.md M16): такое сочетание проходит
